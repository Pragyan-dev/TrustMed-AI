...
try:
    ...
    sample = chain.graph.query("MATCH (d:Disease) WHERE d.cui IS NOT NULL RETURN d.name, d.cui LIMIT 5")
    
    print(f"DISEASE_COUNT: {dis_count}")
    print(f"SYMPTOM_COUNT: {sym_count}")
    print(f"SAMPLES: {sample}")
    
    # Check if Atelectasis from earlier is still there or updated
    atelect_check = chain.graph.query("MATCH (d:Disease {name: 'Atelectasis'}) RETURN d.name, d.cui")
