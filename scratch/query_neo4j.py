...
try:
    ...
    results = chain.graph.query(
        "MATCH (n)-[r]->(m) WHERE toLower(n.name) CONTAINS 'atelect' "
        "RETURN n.name, type(r), m.name LIMIT 10"
    )
    print(f"Graph Results: {results}")

    q_results = chain.graph.query(
        "MATCH (n) WHERE toLower(n.name) CONTAINS 'pneumonia' RETURN n.name LIMIT 10"
    )
